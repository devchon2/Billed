...
`,h=e=>{let t;return("string"==typeof(t=JSON.parse(localStorage.getItem("user")))&&(t=JSON.parse(t)),t&&"Employee"===t.type)?`
...
</svg>`,b=e=>{let t=new Date(null!==e?e:"1970-01-01").toLocaleDateString("fr-FR",{year:"numeric",month:"short",day:"numeric"}).split(" "),a=t[0].padStart(2,"0"),i=t[1],l=t[2].padStart(4,"0"),s=i.charAt(0).toUpperCase()+i.slice(1);return`${a} ${s} ${l}`},g=e=>{let t=new Date(e),a=new Intl.DateTimeFormat("us",{year:"numeric"}).format(t),i=new Intl.DateTimeFormat("us",{month:"2-digit"}).format(t),l=new Intl.DateTimeFormat("us",{day:"2-digit"}).format(t);return Number(`${a}${i}${l}`)},f=e=>{switch(e){case"pending":return"En attente";case"accepted":return"Accept\xe9";case"refused":return"Refused"}};function w(e){let{data:t,loading:a,error:i}=e,l=t&&t.length>0?t:[];return a?p():i?u(i):`
    <div class='layout'>
      ${h(120)}
      <div class='content'>
        <div class='content-header'>
        ...
        <table id="example" class="table table-striped" style="width:100%">
          ...
          <tbody data-testid="tbody">
            ${l.sort((e,t)=>g(t.date)-g(e.date)).map(e=>{let t;return`
    <tr>
      <td>${e.type}</td>
      <td>${e.name}</td>
      <td data-testid='bill-date'>${b(e.date)}</td>
      <td>${e.amount} €</td>
      <td>${e.status}</td>
      <td >
...
`,E=["cedric.hiely@billed.com","christian.saluzzo@billed.com","jean.limbert@billed.com","joanna.binet@billed.com"];class L{constructor({document:e,onNavigate:t,localStorage:i}){a(this,"handleClick",e=>{e.preventDefault(),this.localStorage.clear(),this.onNavigate(B.Login)}),this.document=e,this.localStorage=i,this.onNavigate=t,$("#layout-disconnect").click(this.handleClick)}}let M=(e,t)=>e&&e.length?e.filter(e=>{let a;if("u">typeof jest)a=e.status===t;else{let i=JSON.parse(localStorage.getItem("user")).email;a=e.status===t&&![...E,i].includes(e.email)}return a}):[],N=e=>{switch(e){case 1:return"pending";case 2:return"accepted";case 3:return"refused"}};class T{handleEditTicket(e,t,a){if((void 0===this.counter||this.id!==t.id)&&(this.counter=0),(void 0===this.id||this.id!==t.id)&&(this.id=t.id),this.counter%2==0)a.forEach(e=>{$(`#open-bill${e.id}`).css({background:"#0D5AE5"})}),$(`#open-bill${t.id}`).css({background:"#2A2B35"}),$(".dashboard-right-container div").html(`
        ...
        <div class="col-sm">
          <label for="file" class="bold-label">Justificatif</label>
            ...
            <div class='icons-container'>
              <span id="icon-eye-d" data-testid="icon-eye-d" data-bill-url="${t.fileUrl}"> ${C} </span>
            </div>
          </div>
        </div>
      </div>
  ...
  `}).join(""):""),this.counter++}else $(`#arrow-icon${this.index}`).css({transform:"rotate(90deg)"}),$(`#status-bills-container${this.index}`).html("");return t.forEach(e=>{e.status===N(this.index)&&$(`#open-bill${e.id}`).click(a=>this.handleEditTicket(a,e,t))}),t}constructor({document:e,onNavigate:t,store:i,bills:l,localStorage:s}){a(this,"handleClickIconEye",()=>{let e=$("#icon-eye-d").attr("data-bill-url"),t=Math.floor(.8*$("#modaleFileAdmin1").width());$("#modaleFileAdmin1").find(".modal-body").html(`<div style='text-align: center;'><img width=${t} src=${e} alt="Bill"/></div>`),"function"==typeof $("#modaleFileAdmin1").modal&&$("#modaleFileAdmin1").modal("show")}),a(this,"handleAcceptSubmit",(e,t)=>{let a={...t,status:"accepted",commentAdmin:$("#commentary2").val()};this.updateBill(a),this.onNavigate(B.Dashboard)}),a(this,"handleRefuseSubmit",(e,t)=>{let a={...t,status:"refused",commentAdmin:$("#commentary2").val()};this.updateBill(a),this.onNavigate(B.Dashboard)}),a(this,"getBillsAllUsers",()=>{if(this.store)return this.store.bills().list().then(e=>e.map(e=>({id:e.id,...e,date:e.date,status:e.status}))).catch(e=>{throw e})}),a(this,"updateBill",e=>{if(this.store)return this.store.bills().update({data:JSON.stringify(e),selector:e.id}).then(e=>e).catch(console.log)}),this.document=e,this.onNavigate=t,this.store=i,$("#arrow-icon1").click(e=>this.handleShowTickets(e,l,1)),$("#arrow-icon2").click(e=>this.handleShowTickets(e,l,2)),$("#arrow-icon3").click(e=>this.handleShowTickets(e,l,3)),new L({localStorage:s,onNavigate:t})}}let I=`
  <svg xmlns="http://www.w3.org/2000/svg"
...
`,A=e=>{let{data:t,loading:a,error:i}=e;return a?p():i?u(i):`
    <div class='layout '>
      ${h(120)}
      <div class='dashboard-content' data-testid='dashboard-content'>
        <div class='bills-feed'>
          <div class='status-bills-header'>
            <h3> En attente (${M(t&&t.bills,"pending").length}) </h3>
            <span class='arrow-icon' id='arrow-icon1' data-testid='arrow-icon1'>${I}</span>
          </div>
          <div class='status-bills-container' id='status-bills-container1'>
          </div>
          
            <div class='status-bills-header' style='margin-top: 20px;'>
              <h3> Valid\xe9 (${M(t&&t.bills,"accepted").length}) </h3>
              <span class='arrow-icon' id='arrow-icon2' data-testid='arrow-icon2'>${I}</span>
            </div>
            <div class='status-bills-container' id='status-bills-container2'>
            </div>

            <div class='status-bills-header' style='margin-top: 20px;'>
              <h3> Refus\xe9 (${M(t&&t.bills,"refused").length}) </h3>
              <span class='arrow-icon' id='arrow-icon3' data-testid='arrow-icon3'>${I}</span>
            </div>
    ...
    </div>`},B={Login:"/",Bills:"#employee/bills",NewBill:"#employee/bill/new",Dashboard:"#admin/dashboard"},D=e=>{let{pathname:t,data:a,error:i,loading:l}=e;switch(t){case B.Login:return n({data:a,error:i,loading:l});case B.Bills:return w({data:a,error:i,loading:l});case B.NewBill:return`
    <div class='layout'>
      ${h(120)}
      <div class='content'>
        <div class='content-header'>
  ...
  `;case B.Dashboard:return A({data:a,error:i,loading:l});default:return n({data:a,error:i,loading:l})}},O="";class U{constructor({document:e,localStorage:t,onNavigate:i,PREVIOUS_LOCATION:l,store:s}){a(this,"handleSubmitEmployee",e=>{e.preventDefault();let t={type:"Employee",email:e.target.querySelector('input[data-testid="employee-email-input"]').value,password:e.target.querySelector('input[data-testid="employee-password-input"]').value,status:"connected"};this.localStorage.setItem("user",JSON.stringify(t)),this.login(t).catch(e=>this.createUser(t)).then(()=>{this.onNavigate(B.Bills),this.PREVIOUS_LOCATION=B.Bills,O=this.PREVIOUS_LOCATION,this.document.body.style.backgroundColor="#fff"})}),a(this,"handleSubmitAdmin",e=>{e.preventDefault();let t={type:"Admin",email:e.target.querySelector('input[data-testid="admin-email-input"]').value,password:e.target.querySelector('input[data-testid="admin-password-input"]').value,status:"connected"};this.localStorage.setItem("user",JSON.stringify(t)),this.login(t).catch(e=>this.createUser(t)).then(()=>{this.onNavigate(B.Dashboard),this.PREVIOUS_LOCATION=B.Dashboard,O=this.PREVIOUS_LOCATION,document.body.style.backgroundColor="#fff"})}),a(this,"login",e=>this.store?this.store.login(JSON.stringify({email:e.email,password:e.password})).then(e=>{let{jwt:t}=e;localStorage.setItem("jwt",t)}):null),a(this,"createUser",e=>this.store?this.store.users().create({data:JSON.stringify({type:e.type,name:e.email.split("@")[0],email:e.email,password:e.password})}).then(()=>(console.log(`User with ${e.email} is created`),this.login(e))):null),this.document=e,this.localStorage=t,this.onNavigate=i,this.PREVIOUS_LOCATION=l,this.store=s,this.document.querySelector('form[data-testid="form-employee"]').addEventListener("submit",this.handleSubmitEmployee),this.document.querySelector('form[data-testid="form-admin"]').addEventListener("submit",this.handleSubmitAdmin)}}class q{constructor({document:e,onNavigate:t,store:i,localStorage:l}){a(this,"handleClickNewBill",()=>{this.onNavigate(B.NewBill)}),a(this,"handleClickIconEye",e=>{let t=e.getAttribute("data-bill-url"),a=Math.floor(.5*$("#modaleFile").width());$("#modaleFile").find(".modal-body").html(`<div style='text-align: center;' class="bill-proof-container"><img width=${a} src=${t} alt="Bill" /></div>`),$("#modaleFile").modal("show")}),a(this,"getBills",()=>{if(this.store)return this.store.bills().list().then(e=>e.map(e=>{try{return{...e,date:b(e.date),status:f(e.status)}}catch(t){return console.log(t,"for",e),{...e,date:e.date,status:f(e.status)}}}))}),this.document=e,this.onNavigate=t,this.store=i;const s=e.querySelector('button[data-testid="btn-new-bill"]');s&&s.addEventListener("click",this.handleClickNewBill);const o=e.querySelectorAll('div[data-testid="icon-eye"]');o&&o.forEach(e=>{e.addEventListener("click",()=>this.handleClickIconEye(e))}),new L({document:e,localStorage:l,onNavigate:t})}}class P{constructor({document:e,onNavigate:t,store:i,localStorage:l}){a(this,"handleChangeFile",e=>{e.preventDefault();let t=this.document.querySelector('input[data-testid="file"]'),a=t.files[0],i=a.name;if(!["image/png","image/jpg","image/jpeg","image/gif"].includes(a.type)){t.files=null,t.value=null,a=void 0,i=void 0,alert("Mauvais format de fichier! \n Seuls les fichiers .jpg, .jpeg, .png et .gif sont accept\xe9s");return}let l=new FormData;l.append("file",a),l.append("email",this.email),this.store.bills().create({data:l,headers:{noContentType:!0}}).then(e=>{let{filePath:t,id:a,key:l}=e;this.billId=a,this.key=l,this.path=t.replace("public\\","public/"),this.fileName=i}).catch(e=>console.error(e))}),a(this,"handleSubmit",e=>{e.preventDefault();let t=this.document.querySelector('form[data-testid="form-new-bill"]'),a=`${t.querySelector('select[data-testid="expense-type"]').value}`,i=`${t.querySelector('input[data-testid="expense-name"]').value}`,l=parseInt(t.querySelector('input[data-testid="amount"]').value)||0,s=t.querySelector('input[data-testid="datepicker"]').value.toString(),o=`${t.querySelector('input[data-testid="vat"]').value.toString()||"0"}`,d=parseInt(t.querySelector('input[data-testid="pct"]').value)||20,n=`${t.querySelector('textarea[data-testid="commentary"]').value}`,r={name:i,type:a,email:this.email,date:s,vat:o,pct:d,commentary:n,status:"pending",amount:l,fileName:this.fileName,fileUrl:this.path};this.updateBill(r)}),a(this,"updateBill",e=>{this.store&&this.store.bills().update({data:JSON.stringify(e),selector:this.key}).then(()=>{this.onNavigate(B.Bills)}).catch(e=>console.error(e))}),this.document=e,this.onNavigate=t,this.store=i,this.document.querySelector('form[data-testid="form-new-bill"]').addEventListener("submit",this.handleSubmit),this.document.querySelector('input[data-testid="file"]').addEventListener("change",this.handleChangeFile),this.path=null,this.user=JSON.parse(l.getItem("user")),this.email=this.user.email,new L({document:e,localStorage:l,onNavigate:t})}}!function a(i){var l=t[i];if(void 0!==l)return l.exports;var s=t[i]={exports:{}};return e[i](s,s.exports,a),s.exports}(884),(()=>{let e="/Billed/",t=()=>{let t=window.location.pathname;return t.startsWith(e.replace(/\/$/,""))?t.slice(e.replace(/\/$/,"").length)||"/":t},a=document.getElementById("root");if(a.innerHTML=D({pathname:t()}),window.onNavigate=e=>{if(window.history.pushState({},e,window.location.origin+e),e===B.Login)a.innerHTML=D({pathname:e}),document.body.style.backgroundColor="#0E5AE5",new U({document,localStorage,onNavigate,PREVIOUS_LOCATION:O,store:d});else if(e===B.Bills){a.innerHTML=D({pathname:e,loading:!0});let t=document.getElementById("layout-icon1"),i=document.getElementById("layout-icon2");t.classList.add("active-icon"),i.classList.remove("active-icon"),new q({document,onNavigate,store:d,localStorage}).getBills().then(e=>{a.innerHTML=w({data:e});let t=document.getElementById("layout-icon1"),i=document.getElementById("layout-icon2");t.classList.add("active-icon"),i.classList.remove("active-icon"),new q({document,onNavigate,store:d,localStorage})}).catch(t=>{a.innerHTML=D({pathname:e,error:t})})}else if(e===B.NewBill){a.innerHTML=D({pathname:e,loading:!0}),new P({document,onNavigate,store:d,localStorage});let t=document.getElementById("layout-icon1"),i=document.getElementById("layout-icon2");t.classList.remove("active-icon"),i.classList.add("active-icon")}else e===B.Dashboard&&(a.innerHTML=D({pathname:e,loading:!0}),new T({document,onNavigate,store:d,bills:[],localStorage}).getBillsAllUsers().then(e=>{a.innerHTML=A({data:{bills:e}}),new T({document,onNavigate,store:d,bills:e,localStorage})}).catch(t=>{a.innerHTML=D({pathname:e,error:t})}))},window.onpopstate=e=>{e.preventDefault();let i=JSON.parse(localStorage.getItem("user"));"/"!==t()||i?i&&onNavigate(O):(document.body.style.backgroundColor="#0E5AE5",a.innerHTML=D({pathname:t()}))},"/"===t()&&""===window.location.hash)new U({document,localStorage,onNavigate,PREVIOUS_LOCATION:O,store:d}),document.body.style.backgroundColor="#0E5AE5";else if(""!==window.location.hash)if(window.location.hash===B.Bills){a.innerHTML=D({pathname:window.location.hash,loading:!0});let e=document.getElementById("layout-icon1"),t=document.getElementById("layout-icon2");e.classList.add("active-icon"),t.classList.remove("active-icon"),new q({document,onNavigate,store:d,localStorage}).getBills().then(e=>{a.innerHTML=w({data:e});let t=document.getElementById("layout-icon1"),i=document.getElementById("layout-icon2");t.classList.add("active-icon"),i.classList.remove("active-icon"),new q({document,onNavigate,store:d,localStorage})}).catch(e=>{a.innerHTML=D({pathname:window.location.hash,error:e})})}else if(window.location.hash===B.NewBill){a.innerHTML=D({pathname:window.location.hash,loading:!0}),new P({document,onNavigate,store:d,localStorage});let e=document.getElementById("layout-icon1"),t=document.getElementById("layout-icon2");e.classList.remove("active-icon"),t.classList.add("active-icon")}else window.location.hash===B.Dashboard&&(a.innerHTML=D({pathname:window.location.hash,loading:!0}),new T({document,onNavigate,store:d,bills:[],localStorage}).getBillsAllUsers().then(e=>{a.innerHTML=A({data:{bills:e}}),new T({document,onNavigate,store:d,bills:e,localStorage})}).catch(e=>{a.innerHTML=D({pathname:window.location.hash,error:e})}))})()})()})();