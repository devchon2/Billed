...
`,E=e=>{let{data:t,loading:a,error:i}=e;return a?m():i?c(i):`
    ...
    </div>`},I={Login:"/",Bills:"#employee/bills",NewBill:"#employee/bill/new",Dashboard:"#admin/dashboard"},T=e=>{let{pathname:t,data:a,error:i,loading:l}=e;switch(t){case I.Login:return s({data:a,error:i,loading:l});case I.Bills:return b({data:a,error:i,loading:l});case I.NewBill:return`
    <div class='layout'>
      ${r(120)}
      <div class='content'>
        <div class='content-header'>
  ...
  `;case I.Dashboard:return E({data:a,error:i,loading:l});default:return s({data:a,error:i,loading:l})}},A="";class B{constructor({document:e,localStorage:t,onNavigate:a,PREVIOUS_LOCATION:i,store:s}){l(this,"handleSubmitEmployee",e=>{e.preventDefault();let t={type:"Employee",email:e.target.querySelector('input[data-testid="employee-email-input"]').value,password:e.target.querySelector('input[data-testid="employee-password-input"]').value,status:"connected"};this.localStorage.setItem("user",JSON.stringify(t)),this.login(t).catch(e=>this.createUser(t)).then(()=>{this.onNavigate(I.Bills),this.PREVIOUS_LOCATION=I.Bills,A=this.PREVIOUS_LOCATION,this.document.body.style.backgroundColor="#fff"})}),l(this,"handleSubmitAdmin",e=>{e.preventDefault();let t={type:"Admin",email:e.target.querySelector('input[data-testid="admin-email-input"]').value,password:e.target.querySelector('input[data-testid="admin-password-input"]').value,status:"connected"};this.localStorage.setItem("user",JSON.stringify(t)),this.login(t).catch(e=>this.createUser(t)).then(()=>{this.onNavigate(I.Dashboard),this.PREVIOUS_LOCATION=I.Dashboard,A=this.PREVIOUS_LOCATION,document.body.style.backgroundColor="#fff"})}),l(this,"login",e=>this.store?this.store.login(JSON.stringify({email:e.email,password:e.password})).then(e=>{let{jwt:t}=e;localStorage.setItem("jwt",t)}):null),l(this,"createUser",e=>this.store?this.store.users().create({data:JSON.stringify({type:e.type,name:e.email.split("@")[0],email:e.email,password:e.password})}).then(()=>(console.log(`User with ${e.email} is created`),this.login(e))):null),this.document=e,this.localStorage=t,this.onNavigate=a,this.PREVIOUS_LOCATION=i,this.store=s,this.document.querySelector('form[data-testid="form-employee"]').addEventListener("submit",this.handleSubmitEmployee),this.document.querySelector('form[data-testid="form-admin"]').addEventListener("submit",this.handleSubmitAdmin)}}class D{constructor({document:e,onNavigate:t,store:a,localStorage:i}){l(this,"handleClickNewBill",()=>{this.onNavigate(I.NewBill)}),l(this,"handleClickIconEye",e=>{let t=e.getAttribute("data-bill-url"),a=Math.floor(.5*$("#modaleFile").width());$("#modaleFile").find(".modal-body").html(`<div style='text-align: center;' class="bill-proof-container"><img width=${a} src=${t} alt="Bill" /></div>`),$("#modaleFile").modal("show")}),l(this,"getBills",()=>{if(this.store)return this.store.bills().list().then(e=>e.map(e=>{try{return{...e,date:u(e.date),status:v(e.status)}}catch(t){return console.log(t,"for",e),{...e,date:e.date,status:v(e.status)}}}))}),this.document=e,this.onNavigate=t,this.store=a;const s=e.querySelector('button[data-testid="btn-new-bill"]');s&&s.addEventListener("click",this.handleClickNewBill);const o=e.querySelectorAll('div[data-testid="icon-eye"]');o&&o.forEach(e=>{e.addEventListener("click",()=>this.handleClickIconEye(e))}),new k({document:e,localStorage:i,onNavigate:t})}}class O{constructor({document:e,onNavigate:t,store:a,localStorage:i}){l(this,"handleChangeFile",e=>{e.preventDefault();let t=this.document.querySelector('input[data-testid="file"]'),a=t.files[0],i=a.name;if(!["image/png","image/jpg","image/jpeg","image/gif"].includes(a.type)){t.files=null,t.value=null,a=void 0,i=void 0,alert("Mauvais format de fichier! \n Seuls les fichiers .jpg, .jpeg, .png et .gif sont accept\xe9s");return}let l=new FormData;l.append("file",a),l.append("email",this.email),this.store.bills().create({data:l,headers:{noContentType:!0}}).then(e=>{let{filePath:t,id:a,key:l}=e;this.billId=a,this.key=l,this.path=t.replace("public\\","public/"),this.fileName=i}).catch(e=>console.error(e))}),l(this,"handleSubmit",e=>{e.preventDefault();let t=this.document.querySelector('form[data-testid="form-new-bill"]'),a=`${t.querySelector('select[data-testid="expense-type"]').value}`,i=`${t.querySelector('input[data-testid="expense-name"]').value}`,l=parseInt(t.querySelector('input[data-testid="amount"]').value)||0,s=t.querySelector('input[data-testid="datepicker"]').value.toString(),o=`${t.querySelector('input[data-testid="vat"]').value.toString()||"0"}`,d=parseInt(t.querySelector('input[data-testid="pct"]').value)||20,n=`${t.querySelector('textarea[data-testid="commentary"]').value}`,r={name:i,type:a,email:this.email,date:s,vat:o,pct:d,commentary:n,status:"pending",amount:l,fileName:this.fileName,fileUrl:this.path};this.updateBill(r)}),l(this,"updateBill",e=>{this.store&&this.store.bills().update({data:JSON.stringify(e),selector:this.key}).then(()=>{this.onNavigate(I.Bills)}).catch(e=>console.error(e))}),this.document=e,this.onNavigate=t,this.store=a,this.document.querySelector('form[data-testid="form-new-bill"]').addEventListener("submit",this.handleSubmit),this.document.querySelector('input[data-testid="file"]').addEventListener("change",this.handleChangeFile),this.path=null,this.user=JSON.parse(i.getItem("user")),this.email=this.user.email,new k({document:e,localStorage:i,onNavigate:t})}}!function a(i){var l=t[i];if(void 0!==l)return l.exports;var s=t[i]={exports:{}};return e[i](s,s.exports,a),s.exports}(884),(()=>{let e=document.getElementById("root");if(e.innerHTML=T({pathname:window.location.pathname}),window.onNavigate=t=>{if(window.history.pushState({},t,window.location.origin+t),t===I.Login)e.innerHTML=T({pathname:t}),document.body.style.backgroundColor="#0E5AE5",new B({document,localStorage,onNavigate,PREVIOUS_LOCATION:A,store:i});else if(t===I.Bills){e.innerHTML=T({pathname:t,loading:!0});let a=document.getElementById("layout-icon1"),l=document.getElementById("layout-icon2");a.classList.add("active-icon"),l.classList.remove("active-icon"),new D({document,onNavigate,store:i,localStorage}).getBills().then(t=>{e.innerHTML=b({data:t});let a=document.getElementById("layout-icon1"),l=document.getElementById("layout-icon2");a.classList.add("active-icon"),l.classList.remove("active-icon"),new D({document,onNavigate,store:i,localStorage})}).catch(a=>{e.innerHTML=T({pathname:t,error:a})})}else if(t===I.NewBill){e.innerHTML=T({pathname:t,loading:!0}),new O({document,onNavigate,store:i,localStorage});let a=document.getElementById("layout-icon1"),l=document.getElementById("layout-icon2");a.classList.remove("active-icon"),l.classList.add("active-icon")}else t===I.Dashboard&&(e.innerHTML=T({pathname:t,loading:!0}),new M({document,onNavigate,store:i,bills:[],localStorage}).getBillsAllUsers().then(t=>{e.innerHTML=E({data:{bills:t}}),new M({document,onNavigate,store:i,bills:t,localStorage})}).catch(a=>{e.innerHTML=T({pathname:t,error:a})}))},window.onpopstate=t=>{t.preventDefault();let a=JSON.parse(localStorage.getItem("user"));"/"!==window.location.pathname||a?a&&onNavigate(A):(document.body.style.backgroundColor="#0E5AE5",e.innerHTML=T({pathname:window.location.pathname}))},"/"===window.location.pathname&&""===window.location.hash)new B({document,localStorage,onNavigate,PREVIOUS_LOCATION:A,store:i}),document.body.style.backgroundColor="#0E5AE5";else if(""!==window.location.hash)if(window.location.hash===I.Bills){e.innerHTML=T({pathname:window.location.hash,loading:!0});let t=document.getElementById("layout-icon1"),a=document.getElementById("layout-icon2");t.classList.add("active-icon"),a.classList.remove("active-icon"),new D({document,onNavigate,store:i,localStorage}).getBills().then(t=>{e.innerHTML=b({data:t});let a=document.getElementById("layout-icon1"),l=document.getElementById("layout-icon2");a.classList.add("active-icon"),l.classList.remove("active-icon"),new D({document,onNavigate,store:i,localStorage})}).catch(t=>{e.innerHTML=T({pathname:window.location.hash,error:t})})}else if(window.location.hash===I.NewBill){e.innerHTML=T({pathname:window.location.hash,loading:!0}),new O({document,onNavigate,store:i,localStorage});let t=document.getElementById("layout-icon1"),a=document.getElementById("layout-icon2");t.classList.remove("active-icon"),a.classList.add("active-icon")}else window.location.hash===I.Dashboard&&(e.innerHTML=T({pathname:window.location.hash,loading:!0}),new M({document,onNavigate,store:i,bills:[],localStorage}).getBillsAllUsers().then(t=>{e.innerHTML=E({data:{bills:t}}),new M({document,onNavigate,store:i,bills:t,localStorage})}).catch(t=>{e.innerHTML=T({pathname:window.location.hash,error:t})}))})()})()})();